...
export class NodeManager {
    ...
    getNodeGroup(key) { return this.views.get(key)?.group; }
    toggleLabels() {
        this.labelsVisible = !this.labelsVisible;
        for (const v of this.views.values())
            v.labelSprite.visible = this.labelsVisible;
    }
    setLabelsVisible(v) {
        this.labelsVisible = v;
        for (const view of this.views.values())
            view.labelSprite.visible = v;
    }
    async sync(config, positions) {
